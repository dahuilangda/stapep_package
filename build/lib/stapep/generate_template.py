...
def map_to_amber_mc(molecule, 
    ...
    for atom in molecule.GetAtoms():
        ...
        if atom.GetSymbol() == 'N' and atom.GetTotalDegree() == 3 and \
            ...
            amber_mapping['HEAD_NAME'] = atom.GetPDBResidueInfo().GetName()
        ...
        # C原子的度为4, 相邻原子为=O，-O，C
            ...
            len([n for n in atom.GetNeighbors() if n.GetSymbol() == 'C']) == 1:
            amber_mapping['TAIL_NAME'] = atom.GetPDBResidueInfo().GetName()
        ...
        if atom.GetSymbol() == 'C' and \
            ...
            amber_mapping['MAIN_CHAIN'] = atom.GetPDBResidueInfo().GetName()

    # 开始获取OMIT_NAME, 
    # 1. 与TAIL_NAME相邻的OH原子，包括O原子及其相邻的H原子
    for atom in molecule.GetAtoms():
        if atom.GetPDBResidueInfo().GetName() == amber_mapping['TAIL_NAME']:
            for n in atom.GetNeighbors():
                if n.GetSymbol() == 'O' and len([n for n in n.GetNeighbors() if n.GetSymbol() == 'H']) == 1:
                    ...
                    amber_mapping['OMIT_NAME'].extend([n.GetPDBResidueInfo().GetName() for n in n.GetNeighbors() if n.GetSymbol() == 'H'])

    for atom in molecule.GetAtoms():
        if atom.GetPDBResidueInfo().GetName() == amber_mapping['HEAD_NAME']:
...
def mc_to_prepin(ac, mc, prepin=None):
    """将Amber mc文件转换为prepin文件"""
    '''
    prepgen -i R1A.ac -o R1A.prepin -m R1A.mc -rn R1A
    '''
    cmd = f'prepgen -i {ac} -o {prepin} -m {mc} -rn {mc}'
    print(cmd)
    os.system(cmd)
    return prepin
...
if __name__ == '__main__':
    ...
    mol = read_molecule_from_pdb(pdb)
    amber_map = map_to_amber_mc(mol, matched_atoms, charge=args.charge)
    print(f'amber_map: {amber_map}')
